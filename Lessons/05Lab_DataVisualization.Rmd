...
PeterPaul.chem.nutrients %>%
  ggplot() +
  geom_bar(aes(x=month)) +
  scale_x_discrete(drop=F) #Makes sure missing levels aren't dropped from final plot
...
PeterPaul.chem.nutrients %>%
  ...
  facet_wrap(facets= vars(lakename),ncol = 2, nrow = 1)+ ylab("Temperature [C]") + xlab("Number of Days") + labs(color="Year")
...
PeterPaul.chem.nutrients %>%
   ggplot(aes(
    x=sampledate,
    y=temperature_C,
    color=depth)) +geom_point(size=.5) + scale_color_viridis(option = "magma", direction = -1) + xlab("Day Number") + ylab("Temperature [C]")+ labs(color= "Depth") + scale_x_date(date_breaks = "5 years", date_labels = "%Y")
...
EPAair %>%
  ggplot(aes(
    ...
    y=Ozone,
    color=mean_Lat
  ), alpha=0.5) + geom_point() + geom_smooth(method = lm, se=FALSE)
