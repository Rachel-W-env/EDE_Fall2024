...
glimpse(NTL.phys.data$sampledate) #Another way to look at data structure
...
#%in% is used for factors instead of ==
...
#Uses a slice. 152 and 304 were chosen bc they are June-October days
```

```{r Exercise 1 & 2}
# Exercise 1: 
# filter NTL.phys.data for the year 1999
# what code do you need to use, based on the class of the variable? integer
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 ==1999)
#Or NTL.phys.data %>% filter(year4==1999) for pipe method

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.Tuesday <- filter(NTL.phys.data, (
  lakeid== "T") & year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: We don't filter using row numbers because
...
NTL.phys.data.tuesdayjuly <-
  NTL.phys.data %>%
  filter(lakeid=="T" & year4 %in% c(1990:1999)) %>%
