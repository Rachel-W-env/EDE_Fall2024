...
#Finds the max value in vector 1
max(vector1)
...
#Vector 1 values multiplied by vector 4 values
vector1 * vector4

#Vector 1 values multiplied by 10
vector1 * 10

#Changes to uppercase letters
toupper(vector2) 

#Reverses values (denying them)
!vector3
...
```{r Creating Matrices}
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) #This fills matrix by column
matrix1
#matrix1 has _5_ rows and _4_ columns

matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #This fills matrix by row first
matrix2
...
matrix1[4, 1] #Row 4 column 1 of matrix 1

matrix1[4,  ] #All elements on row 4

matrix1[ , 3] #All elements in column 3

matrix1[c(12, 14)] #Element 12 and 14

matrix1[c(12:14)] #Elements 12-14

matrix1[2:4, 1:3] #Submatrix  
...
#Gives only the age element
list1$age

#Gives all element names in the list
names(list1)

#Gives the first two elements in the list
list1[1:2]

#Changes name from Maria to Janelle
list1$name = "Janelle"

#Changes the 4th element (age) to 44
list1[[4]] = 44
```
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #Creates a table visualization of the dataframe
...
>>>>>>> 5f9541b6f5d270ecb1a71599ebcd219dc5a38f6a
```