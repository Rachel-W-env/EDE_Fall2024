...
ggplot(data = USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar()
#Unique shows how many unique values are in a dataset
#A means approved, A;e and P have other meanings
#geom_graph type is the format
```
...
```{r, fig.height = 3, fig.width = 4}
# Range of values in discharge mean. y axis is count of rows falling into this grouping.
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# Binwidth is now 10 units of discharge. This makes the graph smaller.
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# 20 bins across the data
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# This changes the limits (min and max) of the x-axis.
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# Looks at a different value.
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# Many plots added together. lty is line type (can make it dashed)
ggplot(USGS.flow.data) +
...
#Whn the color is inside the aesthetic it is a categorical variable with unique color
ggplot(USGS.flow.data) +
