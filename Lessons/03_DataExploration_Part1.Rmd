...
#Install packages - uncomment these only when you need to install packages. Knitting can't work with install packages
...
> Answer: WHen you load a package it is enabled for your current R-script. 
...
str(USGS.flow.data) #Shows the structure/size of the dataframe
dim(USGS.flow.data) #Shows dimensions (rows and columns)
length(USGS.flow.data)

head(USGS.flow.data)
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #Shows specific subsample. Rows 3000:30005 and columns 3, 8, and 14
...
> Answer: They were listed as N/A
...
summary(USGS.flow.data$gage.height.mean) #Summary will list the amount of N/As in a dataset
```
