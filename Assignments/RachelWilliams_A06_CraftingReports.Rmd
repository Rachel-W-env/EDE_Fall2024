...
`EPAair_O3_PM25_NC1819_Processed.csv`, `2018-2019`, and `EPA Air Quality System (AQS)`.
...
Date | 2018-2019 
Source | EPA Air Quality System (AQS) | Source
...
summary_PM25$mean_PM2.5 <- round(summary_PM25$mean_PM2.5, 2)
summary_Ozone$mean_Ozone <- round(summary_Ozone$mean_Ozone, 2)
...
```{r fig.height= 5, fig.width= 10, fig.cap= "Ozone by Month 2018", fig.align= "left", echo=FALSE, message=FALSE, warning=FALSE}
#Ozone 2018 by month

EPA_2018 <- filter(epa_data, Year == 2018)
EPA_2018$Month <-factor(EPA_2018$Month,
       levels= 1:12,
       labels = month.abb)

ozone_2018 <- EPA_2018 %>%
  ggplot(aes(x=Month, y=Ozone)) + geom_boxplot() +ggtitle("Ozone 2018") + theme(plot.title=element_text(size=18), axis.title.x = element_text(size = 14),
    axis.title.y = element_text(size = 14), 
    axis.text = element_text(size = 12))
ozone_2018
```

```{r fig.height=5, fig.width=10, fig.cap= "Ozone by Month 2019", fig.align="left", echo=FALSE, message=FALSE, warning=FALSE}
#Ozone 2019 by month

EPA_2019 <- filter(epa_data, Year == 2019)
EPA_2019$Month <-factor(EPA_2019$Month,
       levels= 1:12,
       labels = month.abb)

ozone_2019 <- EPA_2019 %>%
  ggplot(aes(x=Month, y=Ozone)) + geom_boxplot() +ggtitle("Ozone 2019") +theme(plot.title=element_text(size=18),  axis.title.x = element_text(size = 14),  
    axis.title.y = element_text(size = 14), 
    axis.text = element_text(size = 12))
ozone_2019
