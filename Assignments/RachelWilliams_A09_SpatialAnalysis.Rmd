...
> ANSWER: The EPSG code of the counties dataset is 4269. This is associated with the NAD83 datum and it is a geodetic corrdinate reference system. It uses angular units and is a geographic coordinate reference system. 
...
> ANSWER: dec_lat_va contains the x coordinate values and dec_long_va contains
...
>ANSWER: The fields of dec_long_va and dec_lat_va disappeared.
A new field called geometry appeared.
...
Nebraska <- ggplot() + 
  ...
  geom_sf(data=NWIS_sf, col= 'red') + 
  labs(title='NWIS Gage Locations in Nebraska',
       subtitle='Rachel Williams')
Nebraska
...
Gage <- read.csv(here('Data/Raw/NWIS_SiteFlowData_NE_RAW.csv'), 
...
joined_gage <- inner_join(NWIS, Gage, by = 'site_no')

#17. Show the column names in the resulting spatial dataframe

colnames(joined_gage)
...
joined_sf <- joined_gage %>%
st_as_sf(coords = c('dec_long_va','dec_lat_va'), crs=4269)

gageplot <- ggplot()+
geom_sf(data = counties_sf, fill = "lightgray") +
  geom_sf(data = joined_sf, aes(color = gage_ht), size = 3)
...
gage_sf_utm <- st_transform(joined_sf, crs = 32614)

#21 Plot the data

gageplot <- ggplot()+
geom_sf(data = counties_sf_utm, fill = "lightgray") +
  geom_sf(data = gage_sf_utm, col='red')
...
geom_sf(data = counties_sf, fill = "lightgray") +
  geom_sf(data=lancaster, fill="yellow") +
    geom_sf(data=gage_15, col="red") +
  geom_sf(data=Lancaster_gage, col="blue")
